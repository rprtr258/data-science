# data-science
Struggling to learn anything

### DataMining_2019_job1. Выбрать датасеты для кластеризации и анализа временных рядов
[Цены алмазов](https://nbviewer.jupyter.org/github/rprtr258/data-science/blob/master/appartment-choosing.ipynb)
[Потребление электричества(TODO)](f)

### DataMining_2019_job2. Посчитать статистики для любого датасета
[ссылка на задание](https://nbviewer.jupyter.org/github/rprtr258/data-science/blob/master/diamonds-dataset.ipynb)

### DataMining_2019_job3. Построить box plot для имен из датасета имен Москвы
[ссылка на задание](https://nbviewer.jupyter.org/github/rprtr258/data-science/blob/master/boxes.ipynb)

### DataMining_2019_job4. Выбрать топ 3 квартиры для Пети
[ссылка на задание](https://nbviewer.jupyter.org/github/rprtr258/data-science/blob/master/appartment-choosing.ipynb)

### DataMining_2019_job6. Интерактивная диаграмма
[ссылка на задание](https://nbviewer.jupyter.org/github/rprtr258/data-science/blob/master/diamonds-interactive-diagram.ipynb)
